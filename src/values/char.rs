...
impl ValueTrait for ValueChar {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::U8(v) => Some(Value::Char(ValueChar::with_value(
                self.value.unwrap() + v.value().unwrap() as u32
            ))),

            Value::U16(v) => Some(Value::Char(ValueChar::with_value(
                self.value.unwrap() + v.value().unwrap() as u32
            ))),

            Value::U32(v) => Some(Value::Char(ValueChar::with_value(
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ...
                ApicaTypeBytecode::U32 => Some(Value::U32(ValueU32::with_value(value))),
