...
impl ValueU16 {
    ...
    pub fn with_value(value: u16) -> ValueU16 {
        ValueU16 { value: Some(value) }
    }
...
impl ValueTrait for ValueU16 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::Bool(v) => Some(Value::U16(ValueU16::with_value(
                self.value.unwrap() + v.value().unwrap() as u16
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U16(ValueU16::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U16(ValueU16::with_value(*val_ref)))
    }
    ...
    fn bitwise_not(&self) -> Option<Value> {
        Some(Value::U16(ValueU16::with_value(
            !self.value.unwrap()
        )))
    }
    ...
    fn bitwise_or(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::U16(ValueU16::with_value(
                self.value.unwrap() | v.value().unwrap() as u16
            ))),
    ...
    fn equals(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::I8(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() == v.value().unwrap() as u16 }
            ))),
    ...
    fn convert(&self, to: &ValueType, is_nullable: bool) -> Option<Value> {
        if let Some(value) = self.value {
            match to.value() { 
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(Box::new(ValueType::new(ApicaTypeBytecode::U16, is_nullable)))),
    ...
    fn auto_convert(&self, to: &ValueType, _is_nullable: bool) -> Option<Value> {
        if let Some(value) = self.value {
            match to.value() { 
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U16 => Some(Value::U16(ValueU16::with_value(value))),
            ...
            match to.value() { 
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U16 => Some(Value::U16(ValueU16::new())),
