...
impl ValueF64 {
    ...
    pub fn with_value(value: f64) -> ValueF64 {
...
impl ValueTrait for ValueF64 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),

            Value::I16(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),

            Value::I32(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),
            ...
            Value::U8(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),

            Value::U16(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),

            Value::U32(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),
            ...
            Value::F32(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),
            ...
            Value::Bool(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as u8 as f64
            ))),

            Value::Char(v) => Some(Value::F64(ValueF64::with_value(
                self.value.unwrap() + v.value().unwrap() as f64
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1.0;
        ...
        Some(Value::F64(ValueF64::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1.0;
        ...
        Some(Value::F64(ValueF64::with_value(*val_ref)))
    }
    ...
    fn bitwise_or(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::I16(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::I32(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::I64(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::U8(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::U16(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::U32(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),
            ...
            Value::F32(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),
            ...
            Value::Bool(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),

            Value::Char(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() as u64 | v.value().unwrap() as u64
            ))),
    ...
    fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::with_type(ApicaTypeBytecode::F64))),
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ApicaTypeBytecode::Any | ApicaTypeBytecode::F64 => Some(Value::F64(ValueF64::with_value(value))),
            ...
            match to {
                ApicaTypeBytecode::Any | ApicaTypeBytecode::F64 => Some(Value::F64(ValueF64::new())),
    ...
    fn copy(&self) -> Value {
        match self.value { 
            Some(val) => Value::F64(ValueF64::with_value(val)),
            None => Value::F64(ValueF64::new()),
        }
    }
}