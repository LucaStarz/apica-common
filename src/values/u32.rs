...
impl ValueU32 {
    ...
    pub fn with_value(value: u32) -> ValueU32 {
...
impl ValueTrait for ValueU32 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::Bool(v) => Some(Value::U32(ValueU32::with_value(
                self.value.unwrap() + v.value().unwrap() as u32
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U32(ValueU32::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U32(ValueU32::with_value(*val_ref)))
    }
    ...
    fn bitwise_not(&self) -> Option<Value> {
        Some(Value::U32(ValueU32::with_value(
            !self.value.unwrap()
        )))
    }
    ...
    fn bitwise_or(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::U32(ValueU32::with_value(
                self.value.unwrap() | v.value().unwrap() as u32
            ))),

            Value::I16(v) => Some(Value::U32(ValueU32::with_value(
                self.value.unwrap() | v.value().unwrap() as u32
            ))),
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ...
                ApicaTypeBytecode::Char => Some(Value::Char(ValueChar::with_value(value))),
