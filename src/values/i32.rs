...
impl ValueI32 {
    ...
    pub fn with_value(value: i32) -> ValueI32 {
        ValueI32 { value: Some(value) }
    }
...
impl ValueTrait for ValueI32 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::I32(ValueI32::with_value(
                self.value.unwrap() + v.value().unwrap() as i32
            ))),
            ...
            Value::Bool(v) => Some(Value::I32(ValueI32::with_value(
                self.value.unwrap() + v.value().unwrap() as i32
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::I32(ValueI32::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::I32(ValueI32::with_value(*val_ref)))
    }
    ...
    fn bitwise_not(&self) -> Option<Value> {
        Some(Value::I32(ValueI32::with_value(
            !self.value.unwrap()
        )))
    }
    ...
    fn equals(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::I8(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() == v.value().unwrap() as i32 }
            ))),
    ...
    fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::with_type(ApicaTypeBytecode::I32))),
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to { 
                ApicaTypeBytecode::Any | ApicaTypeBytecode::I32 => Some(Value::I32(ValueI32::with_value(value))),
            ...
            match to { 
                ApicaTypeBytecode::Any | ApicaTypeBytecode::I32 => Some(Value::I32(ValueI32::new())),
    ...
    fn copy(&self) -> Value {
        match self.value { 
            Some(val) => Value::I32(ValueI32::with_value(val)),
            None => Value::I32(ValueI32::new()),
        }
    }
}