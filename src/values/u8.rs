...
impl ValueU8 {
    ...
    pub fn with_value(value: u8) -> ValueU8 {
        ValueU8 { value: Some(value) }
    }
...
impl ValueTrait for ValueU8 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::Bool(v) => Some(Value::U8(ValueU8::with_value(
                self.value.unwrap() + v.value().unwrap() as u8
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U8(ValueU8::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U8(ValueU8::with_value(*val_ref)))
    }
    ...
    fn bitwise_not(&self) -> Option<Value> {
        Some(Value::U8(ValueU8::with_value(
            !self.value.unwrap()
        )))
    }
    ...
    fn bitwise_or(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::U8(ValueU8::with_value(
                self.value.unwrap() | v.value().unwrap() as u8
            ))),

            Value::I16(v) => Some(Value::U16(ValueU16::with_value(
                self.value.unwrap() as u16 | v.value().unwrap() as u16
            ))),

            Value::I32(v) => Some(Value::U32(ValueU32::with_value(
    ...
    fn equals(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::I8(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() == v.value().unwrap() as u8 }
            ))),

            Value::I16(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() as u16 == v.value().unwrap() as u16 }
            ))),

            Value::I32(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() as u32 == v.value().unwrap() as u32 }
            ))),
            ...
            Value::F32(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() as f32 == v.value().unwrap() }
            ))),
    ...
    fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to { 
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::with_type(ApicaTypeBytecode::U8))),
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U8 => Some(Value::U8(ValueU8::with_value(value))),
            ...
            match to { 
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U8 => Some(Value::U8(ValueU8::new())),
    ...
    fn copy(&self) -> Value {
        match self.value { 
            Some(val) => Value::U8(ValueU8::with_value(val)),
            None => Value::U8(ValueU8::new()),
        }
    }
}