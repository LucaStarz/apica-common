...
impl ValueU64 {
    ...
    pub fn with_value(value: u64) -> ValueU64 {
...
impl ValueTrait for ValueU64 {
    ...
    fn add(&self, other: &Value) -> Option<Value> {
        match other {
            Value::I8(v) => Some(Value::I64(ValueI64::with_value(
                self.value.unwrap() as i64 + v.value().unwrap() as i64
            ))),

            Value::I16(v) => Some(Value::I64(ValueI64::with_value(
                self.value.unwrap() as i64 + v.value().unwrap() as i64
            ))),

            Value::I32(v) => Some(Value::I64(ValueI64::with_value(
                self.value.unwrap() as i64 + v.value().unwrap() as i64
            ))),
            ...
            Value::U8(v) => Some(Value::U64(ValueU64::with_value(
                self.value.unwrap() + v.value().unwrap() as u64
            ))),
    ...
    fn increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U64(ValueU64::with_value(old_value)))
    }
    ...
    fn left_increment(&mut self) -> Option<Value> {
        ...
        *val_ref += 1;
        ...
        Some(Value::U64(ValueU64::with_value(*val_ref)))
    }
    ...
    fn bitwise_not(&self) -> Option<Value> {
        Some(Value::U64(ValueU64::with_value(
            !self.value.unwrap()
        )))
    }
    ...
    fn equals(&self, other: &Value) -> Option<Value> {
        match other {
            ...
            Value::I8(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() == v.value().unwrap() as u64 }
            ))),
            ...
            Value::F32(v) => Some(Value::Bool(ValueBool::with_value(
                if self.is_null() { v.is_null() } else { self.value.unwrap() as f64 == v.value().unwrap() as f64 }
            ))),
    ...
    fn convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to { 
                ...
                ApicaTypeBytecode::Type => Some(Value::Type(ValueType::with_type(ApicaTypeBytecode::U64))),
    ...
    fn auto_convert(&self, to: ApicaTypeBytecode) -> Option<Value> {
        if let Some(value) = self.value {
            match to {
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U64 => Some(Value::U64(ValueU64::with_value(value))),
            ...
            match to {
                ApicaTypeBytecode::Any | ApicaTypeBytecode::U64 => Some(Value::U64(ValueU64::new())),
    ...
    fn copy(&self) -> Value {
        match self.value { 
            Some(val) => Value::U64(ValueU64::with_value(val)),
            None => Value::U64(ValueU64::new()),
        }
    }
}